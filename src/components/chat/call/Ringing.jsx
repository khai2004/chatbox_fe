...
const Ringing = ({ answerCall, endCall, call, setCall }) => {
  const { receivingCall, name, picture, callEnded } = call;
  ...
  }, [call, handleTimer, setCall, timer]);
  return (
    <div
      className='bg-dark_bg_1 rounded-lg fixed top-1/2 left-1/2 -translate-x-1/2
    -translate-y-1/2 shadow-lg z-30
    '
    >
      {/* Container */}
      <div className='p-4 flex items-center justify-between gap-x-8'>
        <img src={picture} alt='caller' className='w-28 h-28 rounded-full' />
        <div>
        ...
        <ul className='flex items-center gap-x-2'>
          ...
          <li onClick={answerCall}>
            ...
            <button className='w-8 h-8 flex items-center justify-center rounded-full  bg-blue-500'>
              <ValidIcon className='fill-white w-6  h-6' />
            </button>
          </li>
        </ul>
      </div>
